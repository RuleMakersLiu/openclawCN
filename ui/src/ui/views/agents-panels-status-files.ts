  ...
  return html`
    <section class="card">
      ...
      <div class="agents-overview-grid" style="margin-top: 16px;">
        <div class="agent-kv">
          <div class="label">${t("agents.label.workspace")}</div>
          <div class="mono">${context.workspace}</div>
        </div>
        <div class="agent-kv">
          <div class="label">${t("agents.label.primaryModel")}</div>
          <div class="mono">${context.model}</div>
        </div>
        <div class="agent-kv">
          <div class="label">${t("agents.label.identityName")}</div>
          <div>${context.identityName}</div>
        </div>
        <div class="agent-kv">
          <div class="label">${t("agents.label.identityEmoji")}</div>
          <div>${context.identityEmoji}</div>
        </div>
        <div class="agent-kv">
          <div class="label">${t("agents.label.skillsFilter")}</div>
          <div>${context.skillsLabel}</div>
        </div>
        <div class="agent-kv">
          <div class="label">${t("agents.label.default")}</div>
...
export function renderAgentChannels(params: {
  ...
  return html`
    <section class="grid grid-cols-2">
      ${renderAgentContextCard(params.context, t("agents.context.overview.sub"))}
      <section class="card">
        <div class="row" style="justify-content: space-between;">
          <div>
            <div class="card-title">${t("agents.channels.title")}</div>
            <div class="card-sub">${t("agents.channels.sub")}</div>
          </div>
          <button class="btn btn--sm" ?disabled=${params.loading} @click=${params.onRefresh}>
            ${params.loading ? t("cron.refreshing") : t("action.refresh")}
          </button>
        </div>
                ...
                <div class="list" style="margin-top: 16px;">
                  ${entries.map((entry) => {
                    ...
                    const status = summary.total
                      ? t("agents.channels.status.connected", summary.connected, summary.total)
                      : t("agents.channels.status.noAccounts");
                    const config = summary.configured
                      ? t("agents.channels.status.configured", summary.configured)
                      : t("agents.channels.status.notConfigured");
                    const enabled = summary.total
                      ? t("agents.channels.status.enabled", summary.enabled)
                      : t("agents.channels.status.disabled");
...
export function renderAgentCron(params: {
  ...
  return html`
    <section class="grid grid-cols-2">
      ${renderAgentContextCard(params.context, t("agents.context.cron.sub"))}
      <section class="card">
        <div class="row" style="justify-content: space-between;">
          <div>
        ...
        <div class="stat-grid" style="margin-top: 16px;">
          <div class="stat">
            ...
            <div class="stat-value">
              ${params.status ? (params.status.enabled ? t("cron.yes") : t("cron.no")) : t("common.na")}
            </div>
          </div>
          <div class="stat">
    ...
    <section class="card">
      <div class="card-title">${t("agents.cronJobs.title")}</div>
      <div class="card-sub">${t("agents.cronJobs.sub")}</div>
      ${
        jobs.length === 0
          ? html`
              <div class="muted" style="margin-top: 16px">${t("agents.cronJobs.none")}</div>
            `
          : html`
              <div class="list" style="margin-top: 16px;">
                ${jobs.map(
  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        ...
        >
          ${params.agentFilesLoading ? t("action.loading") : t("action.refresh")}
        </button>
      </div>
      ${
        list
          ? html`<div class="muted mono" style="margin-top: 8px;">${t("exec.approvals.workspace", list.workspace)}</div>`
          : nothing
      }
      ${
              ...
              <div class="agent-files-grid" style="margin-top: 16px;">
                <div class="agent-files-list">
                  ${
                    files.length === 0
                      ? html`
                          <div class="muted">${t("agents.files.none")}</div>
                        `
                      : files.map((file) =>
                      ...
                      : html`
                          <div class="agent-file-header">
                            ...
                            <div class="agent-file-actions">
                              ...
                              >
                                ${params.agentFileSaving ? t("action.saving") : t("action.save")}
                              </button>
                            </div>
                          </div>
                          ${
  ...
  return html`
    ...
    >
      ...
      ${
        file.missing
          ? html`
              <span class="agent-pill warn">${t("agents.files.pill.missing")}</span>
            `
          : nothing
      }
    </button>
