    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          ...
          <div class="card-sub">${t("nodes.sub")}</div>
        </div>
      ...
      <div class="list" style="margin-top: 16px;">
        ${
          props.nodes.length === 0
            ? html`
                <div class="muted">${t("nodes.none")}</div>
              `
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          ...
          <div class="card-sub">${t("nodes.devices.sub")}</div>
        </div>
        <button class="btn" ?disabled=${props.devicesLoading} @click=${props.onDevicesRefresh}>
          ${props.devicesLoading ? t("action.loading") : t("action.refresh")}
        </button>
      </div>
      ${
        props.devicesError
      ...
      <div class="list" style="margin-top: 16px;">
        ...
        ${
          pending.length === 0 && paired.length === 0
            ? html`
                <div class="muted">${t("nodes.noPaired")}</div>
              `
            : nothing
        }
      </div>
    ...
    <div class="list-item">
      <div class="list-main">
        ...
        ${
          tokens.length === 0
            ? html`
                <div class="muted" style="margin-top: 6px">${t("nodes.tokens.none")}</div>
              `
            : html`
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between; align-items: center;">
        ...
        <button
          ...
          @click=${state.onSave}
        >
          ${state.configSaving ? t("action.saving") : t("action.save")}
        </button>
      </div>
      ...
      ${
        !state.ready
          ? html`<div class="row" style="margin-top: 12px; gap: 12px;">
            <div class="muted">${t("nodes.binding.loadHint")}</div>
            <button class="btn" ?disabled=${state.configLoading} @click=${state.onLoadConfig}>
              ${state.configLoading ? t("action.loading") : t("nodes.load.config")}
            </button>
          </div>`
          : html`
            <div class="list" style="margin-top: 16px;">
              <div class="list-item">
                <div class="list-main">
                  <div class="list-title">${t("nodes.binding.default")}</div>
                  <div class="list-sub">${t("nodes.binding.default.sub")}</div>
                </div>
                <div class="list-meta">
                  <label class="field">
                  ...
                  ${
                    !supportsBinding
                      ? html`
                          <div class="muted">${t("nodes.binding.noExecNodes")}</div>
                        `
                      : nothing
                  }
                </div>
              ...
              ${
                state.agents.length === 0
                  ? html`
                      <div class="muted">${t("nodes.binding.noAgents")}</div>
                    `
    ...
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="chip-row" style="margin-top: 6px;">
          <span class="chip">${paired ? t("nodes.chip.paired") : t("nodes.chip.unpaired")}</span>
          <span class="chip ${connected ? "chip-ok" : "chip-warn"}">
            ${connected ? t("nodes.chip.connected") : t("nodes.chip.offline")}
          </span>
