    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        ...
        <button class="btn" ?disabled=${props.loading} @click=${props.onRefresh}>
          ${props.loading ? t("instances.refreshing") : t("action.refresh")}
        </button>
      </div>
      ${
        props.lastError
      ...
      <div class="list" style="margin-top: 16px;">
        ${
          props.entries.length === 0
            ? html`
                <div class="muted">${t("instances.none")}</div>
              `
  ...
  const scopesLabel =
    scopes.length > 0
      ? scopes.length > 3
        ...
        : t("instances.scopes.label", scopes.join(", "))
      : null;
  return html`
    <div class="list-item">
      <div class="list-main">
        <div class="list-title">${entry.host ?? t("instances.host.unknown")}</div>
